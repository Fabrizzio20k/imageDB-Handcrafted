...
class MetricsEvaluator:
    def __init__(self, retrieval_results_path, queries_csv_path):
        ...
        self.k_values = [5, 10]
        self.mainPath = Path(os.path.join(Path.cwd(), 'Data',
                                          'Results'))
    ...
    def calculate_metrics(self, output_folder='Data/Results'):
        ...
        for _, query_row in self.queries_df.iterrows():
            ...
            relevance = [1 if item ==
                         true_class else 0 for item in retrieved_items]

            for k in self.k_values:
                p_at_k = sum(
                    relevance[:k]) / k if k <= len(relevance) else sum(relevance) / len(relevance)
