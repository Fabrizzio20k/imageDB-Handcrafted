...
class ImageSimilarityIndex:
    ...
    def build_index(self, features_csv_path, output_path='Data/Indexed_feature_vectors'):
        ...
        feature_cols = [
            col for col in df.columns if col not in ['label', 'filename']]
        ...
        print(
            f"Building index with {n_samples} vectors of dimension {dimension}")
        ...
        if self.index_type == 'l2':
            ...
            raise ValueError(
                "index_type must be 'L2', 'IP', 'cosine', 'hnsw', or 'lsh'")
        ...
        index_filename = os.path.join(
            output_path, f'{self.index_type}_index.faiss')
...
if __name__ == "__main__":
    ...
    indexer.build_index(Path(os.path.join(
        Path.cwd(), 'Data', 'Feature_vectors_Caltech_101', 'hog_features.csv')))
