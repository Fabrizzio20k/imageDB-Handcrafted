...
caltechpath = '../Data/caltech-101/101_ObjectCategories' 
ouputpath ='../Data/Feature_vectors_Caltech_101'
class DescriptorOrchestrator:
    def __init__(self, descriptor_type='mixture'):
        ...
        if self.descriptor_type == 'hog':
            ...
            raise ValueError("Descriptor type must be 'hog', 'lbp', 'color', or 'mixture'")
    ...
    def extract_features(self, caltech_path=caltechpath, output_path=ouputpath, batch_size=32, n_workers=None):
        ...
        output_file = os.path.join(output_path, f'{self.descriptor_type}_features.csv')
...
class FastDescriptorOrchestrator:
    ...
    def extract_features(self, caltech_path=caltechpath, output_path=ouputpath, n_workers=None):
        Path(output_path).mkdir(parents=True, exist_ok=True)
        ...
        for category in categories:
            ...
            for img_file in os.listdir(category_path):
                if img_file.lower().endswith(('.png', '.jpg', '.jpeg')):
                    img_path = os.path.join(category_path, img_file)
                    all_tasks.append((img_path, category, img_file, self.descriptor_type))
...
if __name__ == "__main__":
    orchestrator = FastDescriptorOrchestrator(descriptor_type='mixture')
